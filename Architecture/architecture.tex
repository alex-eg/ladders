...
\begin{tikzpicture}[node distance=1cm, auto, ->, >=stealth']
	%nodes
	\node (Sound) [abstract] {Звук};
	\node (Graph) [abstract, right=of Sound] {Графика};	
	\node (UI) [abstract, right=of Graph] {Интерфейс пользователя};
	
	\node (Stat) [abstract, below=of Sound] {Статистика};
	\node (Mistakes) [abstract, right=of Stat] {Ошибки};
	\node (Logs) [abstract, right=of Mistakes] {Логирование};
	
	\node (Physics) [abstract, below=of Stat] {Физика};
	\node (Generators) [abstract, right=of Physics] {Генераторы};
...
\begin{tikzpicture}[node distance=1cm, auto, ->, >=stealth']
	%nodes
	\node (Logic) [abstract] {<<Логика>> игры};
	\node (AV) [abstract, right=of Logic] {Аудио-визуальное сопроводжение игры};	
	\node (UI) [abstract, right=of AV] {Интерфейс пользователя};
...
\begin{tikzpicture}[node distance=1cm, auto, ->, >=stealth']
	%nodes
	\node (Sys) [abstract] {Системный уровень};	
	\node (UI) [abstract, right=of Sys] {Пользовательский уровень};
